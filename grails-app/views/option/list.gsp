			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="optionNumber" title="${message(code: 'option.optionNumber.label', default: 'Option Number')}" />
				...
				<g:each in="${optionInstanceList}" status="i" var="optionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${optionInstance.id}">${fieldValue(bean: optionInstance, field: "optionNumber")}</g:link></td>
					
						<td>${fieldValue(bean: optionInstance, field: "optionText")}</td>
